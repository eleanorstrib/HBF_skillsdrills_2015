...
def common_items(list1, list2):
    ...
    for x in range(0,len(list1)-1):
        if list1_sorted[x] == list1_sorted[x+1]:
            common_list.append(list1_sorted[x])
    for x in range(0,len(list2)-1):
        if list2_sorted[x] == list2_sorted[x+1]:
            common_list.append(list2_sorted[x])

    common_list_sorted = sorted(common_list)




    return common_list_sorted
...
def unique_common_items(list1, list2):
    ...
    unique_items = list1.intersection(list2)
    return unique_items
...
def sum_zero(list1):
    ...
    """
    zero_sum = []
    abs_val_list = [abs(item) for item in list1]
    set_list1 = set(abs_val_list)

    for item in set_list1:
        for thing in list1:
            if item + thing == 0 and [item,thing] not in zero_sum:
                zero_sum.append([item,thing])
    return zero_sum
...
def word_length(words):
    ...
    for word in words:
        ascending_dict[len(word)] = ascending_dict.get(len(word),[]) + [word]

    ascending_list = ascending_dict.items()
...
def adv_word_length_sorted_words(words):
    ...
    for item in ascending_dict:
        ascending_dict.get(item).sort()
...
def pirate_talk(phrase):
    ...
    for word in phrase_list:
        if word in eng_to_pir:
            translation_list.append(eng_to_pir[word])
        else:
            translation_list.append(word)

    translated_phrase = ' '.join(translation_list)
